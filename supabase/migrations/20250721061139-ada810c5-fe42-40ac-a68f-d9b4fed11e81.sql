-- First, clean up duplicates in secondary_sources by keeping only the latest entry per URL
DELETE FROM public.secondary_sources 
WHERE id NOT IN (
  SELECT DISTINCT ON (TRIM(LOWER(url))) id
  FROM public.secondary_sources 
  ORDER BY TRIM(LOWER(url)), discovered_at DESC
);
